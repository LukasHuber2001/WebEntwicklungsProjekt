...
--
DELIMITER $$
CREATE TRIGGER `orders_delete_update_receipt` AFTER DELETE ON `orders` FOR EACH ROW BEGIN
    DECLARE receiptID INT;
    SET receiptID = OLD.receipt_id;

    UPDATE receipt
    SET total = (SELECT SUM(preis * anzahl) FROM orders WHERE r_id = receiptID)
    WHERE id = receiptID;

    IF NOT EXISTS (SELECT 1 FROM orders WHERE r_id = receiptID) THEN
        DELETE FROM receipt WHERE id = receiptID;
    END IF;
END
$$
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `orders_update_receipts` AFTER UPDATE ON `orders` FOR EACH ROW BEGIN
    DECLARE receiptID INT;
...
DELIMITER ;
DELIMITER $$
CREATE TRIGGER `receipt_update_summe` AFTER INSERT ON `orders` FOR EACH ROW BEGIN
    UPDATE receipt
    SET total = (SELECT SUM(preis * anzahl) FROM orderlines WHERE r_id = NEW.receipt_id)
...
--
DELIMITER $$
CREATE TRIGGER `orders_update_anzahl` AFTER INSERT ON `receipt` FOR EACH ROW BEGIN
    UPDATE orders
    SET anzahl = (SELECT SUM(anzahl) FROM orderlines WHERE r_id = NEW.id)
    WHERE r_id = NEW.id;
END
$$
DELIMITER ;
